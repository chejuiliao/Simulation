...
Density.R
Est.R <- rkde(fhat=kde(c(drill1$Return.Crude.Oil,drill1$Return.Natural.Gas,drill1$Return.Dry.Well), h=0.07935), n=n_sims)
Est.Hist = as.data.frame(Est.R)

# drilling cost simulation - Kernel Density -------------------------------
drillin_cost <- rep(0,n_sims)
for (i in 1:n_sims){
  ...
  drillin_cost[i] <- P # outputs cost prediciton for 2019 to vector "cost_k"
}
...
completion_costs <- rnorm(1, mean = 390000, sd=50000)
...
# simulate whether well is wet and the corresponding cost

# whether well is wet
Hydrocarbons = rtruncnorm(n_sims, a=0, b=1, mean=0.99, sd = 0.05)
Reservoir = rtruncnorm(n_sims, a=0, b=1, mean=0.8, sd = 0.1)
...
for (year in 1:n_years) {
  ...
  oil_price <- rtriangle(n = n_wet, a = oil_pred$low_price[year],
